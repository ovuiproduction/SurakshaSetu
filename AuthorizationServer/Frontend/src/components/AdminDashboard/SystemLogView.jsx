        ...
        return (
          <div key={index} className="log-entry">
            <div className="log-header">
              <span className="log-timestamp">{new Date(timestamp).toLocaleString()}</span>
              <span className={`log-method log-method--${method.toLowerCase()}`}>
                {method}
              </span>
              <span className="log-url">{url}</span>
              <span className="log-duration">{duration}ms</span>
              <span className={`log-status log-status--${Math.floor(status/100)}xx`}>
                {status}
              </span>
            </div>
